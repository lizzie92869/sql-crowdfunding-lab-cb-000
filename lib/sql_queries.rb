...
JOIN users ON pledges.user_id = users.id GROUP BY users.name;"
end

def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
"SELECT projects.title, SUM(pledges.amount) - projects.funding_goal  FROM projects
JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING projects.funding_goal <= SUM(pledges.amount);"
end
...
#it shouldn't be WHERE but HAVING because of the SUM...
